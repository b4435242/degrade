...
public:
  ...
  void VisitBinaryOperator(BinaryOperator *Node) {
    std::string op = BinaryOperator::getOpcodeStr(Node->getOpcode()).lower();
    // if (op == "!=") {
    //   OS << "(not (= ";
  ...
  void VisitUnaryOperator(UnaryOperator *Node) {
    std::string op = UnaryOperator::getOpcodeStr(Node->getOpcode()).str();
    if (op == "!") {
      op = "not";
    }
  ...
  void VisitMemberExpr(MemberExpr *Node) {
    ...
    if (Node->hasExplicitTemplateArgs())
      Node->getTemplateArgs()->getArgument().dump(OS);
  }
  ...
  void VisitDeclRefExpr(DeclRefExpr *Node) {
    ...
    if (isa<EnumConstantDecl>(decl)) {
      ...
      if (ecdecl->getInitVal() < 0) {
        OS << "(- ";
        OS << toString((-(ecdecl->getInitVal())), 10);
        OS << ")";
      } else {
        OS << toString(ecdecl->getInitVal(), 10);
      }

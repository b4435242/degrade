...
std::vector<std::vector<int>> vulnerables;
...
void parse_vulnerables(int argc, const char** argv, int offset){
	int n = (argc-offset)/4;
	int count = offset;
	/*
	**vulnerables = new *int(n_vulnerables);
	for(int i=0; i<n_vulnerables; i++){
		vulnerables[i] = new int(4);
		for(int j=0; j<4; j++)
			vulnerables[i][j] = atoi(argv[count++]);
	}*/
	vulnerables.resize(n, std::vector<int>(4));
	for(int i=0; i<n; i++)
		for(int j=0; j<4; j++)
			vulnerables[i][j] = atoi(argv[count++]);	
...
bool isSuspicious(int beginLine, int beginCol, int endLine, int endCol) {
  /*std::string line;
  std::string suspiciousFile("~/degradation/tests/error_handling/vulnerable.txt");
  ...
  for(auto &vul: vulnerables)
	if (vul[0]==beginLine && vul[1]==beginCol && vul[2]==endLine && vul[3]==endCol)
	return true;

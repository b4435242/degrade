...
def build_with_cc(dir, cmd, stderr, cc, config):
    env = dict(os.environ)
    env['CC'] = cc
    logger.info('CC is {}'.format(env['CC']))
...
class Project:
    ...
    def import_compilation_db(self, compilation_db):
        ...
        for item in self.compilation_db:
            ...
            item['arguments'] += ('-include', os.environ['DG_RUNTIME_H'])
            item['arguments'] += ('-D', 'ANGELIX_INSTRUMENTATION')
        compilation_db_file = join(self.dir, 'compile_commands.json')
        with open(compilation_db_file, 'w') as file:
            json.dump(self.compilation_db, file, indent=2)
